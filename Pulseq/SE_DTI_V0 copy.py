...
plot=bool
write_seq=bool
seq_filename="se_dwi_pypulseq_TE65_FOV256_Nx64.seq"
...
delta_k = 1 / fov
k_width = Nx * delta_k
readoutTime=6.3e-4   #TODO #Might be able to change to match bandwidth
readoutTime=6.3e-4
...
delayTR= math.ceil( (TR-pp.calc_duration(gz)  -pp.calc_duration(gx)/2 -TE +gz.fall_time +gz.flat_time/2 )/seq.grad_raster_time)*seq.grad_raster_time
...
# ======
if plot:
    seq.plot(time_range=(7999.9,8000.1))

#%%
rep=seq.test_report()
for items in rep:
    print(items,f'{np.shape(rep[items])}')
