...
def go_generate_maps(CIRC_NUMBER):
    ...
    img_save_dir=os.path.join(defaultPath, "saved_ims_v2_Feb_5_2024","WITH8000_Feb27",CIRC_ID)
    ...
    for ss,obj_T1 in enumerate(MP01_list):
        finalMap,finalRa,finalRb,finalRes=obj_T1.go_ir_fit(searchtype='grid',invertPoint=8)
        plt.figure()
        plt.axis('off')
    ...
    plt.close()
    MP01.imshow_overlay(path=img_save_dir,plot=plot)
    plt.close()
    MP02.imshow_overlay(path=img_save_dir,plot=plot)
    plt.close()
    MP03.imshow_overlay(path=img_save_dir,plot=plot)
