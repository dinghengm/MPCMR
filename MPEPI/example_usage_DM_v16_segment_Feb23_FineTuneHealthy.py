...
#%%
MP01_1.go_ir_fit(invertPoint=12)
MP01_1.imshow_map()
...
MP01_1=mapping(mapList[1])
map=MP01_1
#MP01_2._delete(d=[4,5])
#map._data=np.concatenate((MP01_2._data,MP02._data[:,:,2,0][:,:,np.newaxis,np.newaxis]),axis=-1)
path=rf'C:\Research\MRI\MP_EPI\DataSet\{CIRC_ID}'
if not os.path.exists(path):
    os.makedirs(path)
map._save_nib(path=path)
np.shape(map._data)
print(map.valueList)
#%%
#Load the data
