...
def go_generate_maps(CIRC_NUMBER):
    ...
    saved_img_root_dir= os.path.join(defaultPath, "saved_ims_v2_Feb_5_2024","WITHOUT8000",CIRC_ID)
    ...
    for ss,obj_T1 in enumerate(MP01_list):
        ...
        obj_T1._update()
        valueArray=np.array(obj_T1.valueList)
        #Also cancel the 8000
        arrayInd=np.where(valueArray>=7999)
        obj_T1._delete(d=list(arrayInd))
        obj_T1._update()
        obj_T1.imshow_corrected(ID=f'MP01_Slice{ss}_NULLWithout8000',plot=plot,path=saved_img_root_dir)
        plt.close()
