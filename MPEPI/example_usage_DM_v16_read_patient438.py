...
MP02.go_resize()
newshape=np.shape(MP02._data)[0,1]


for obj in [MP01,MP01_post,MP03]:
    obj.cropzone=cropzone
    obj.go_resize()

for obj in [MP01,MP01_post,MP02,MP03]:
    obj.cropzone=cropzone
    obj.go_resize()
#%%
MP02._update()
MP02.go_create_GIF(path_dir=str(saved_img_root_dir))
MP02.imshow_corrected(ID=f'MP02_Corrected',plot=plot,path=saved_img_root_dir)
MP02.go_t2_fit()

#%%
MP03._update()
MP03.go_create_GIF(path_dir=str(saved_img_root_dir))
MP03.imshow_corrected(ID=f'MP03_Corrected',valueList=MP03.bval,plot=plot,path=saved_img_root_dir)
MP03.go_cal_ADC()
#%%
MP01._update()
MP01.go_create_GIF(path_dir=str(saved_img_root_dir))
MP01.imshow_corrected(ID=f'MP01_Corrected',plot=plot,path=saved_img_root_dir)
MP01.go_ir_fit()
#%%
MP01_post._update()
MP01_post.go_create_GIF(path_dir=str(saved_img_root_dir))
MP01_post.imshow_corrected(ID=f'MP01_post_Corrected',plot=plot,path=saved_img_root_dir)
MP01_post.go_ir_fit()
#%%
for ss in range(MP02.Nz):
    ...
    plt.savefig(img_dir)
MP02.save(filename=os.path.join(saved_img_root_dir,f'{MP02.ID}_p.mapping'))
...
for ss in range(MP03.Nz):
    ...
    plt.savefig(img_dir)
MP03.save(filename=os.path.join(saved_img_root_dir,f'{MP03.ID}_p.mapping'))
#%%
Nx,Ny,Nz,Nd=np.shape(MP01._data)
for ss in range(MP01.Nz):
    ...
    plt.savefig(img_dir)
MP01.save(filename=os.path.join(saved_img_root_dir,f'{MP01.ID}_p.mapping'))
#%%
Nx,Ny,Nz,Nd=np.shape(MP01_post._data)
for ss in range(MP01_post.Nz):
    plt.figure()
    plt.axis('off')
    plt.imshow(MP01_post._map[:,:,ss],cmap='magma',vmin=0,vmax=1600)
    img_dir= os.path.join(saved_img_root_dir,f'Slice{ss}_T1')
    plt.savefig(img_dir)
MP01_post.save(filename=os.path.join(saved_img_root_dir,f'{MP01_post.ID}_p.mapping'))
...
testing_reseg(MP01,MP02,MP03,MP01_post)
