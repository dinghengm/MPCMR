...
# 12 points: [110, 190, 440, 940, 1540, 2140, 2220, 3000, 4000,5000, 6000,8000]


def go_fewerPointsFitting(CIRC_NUMBER):
    
    #Important~~~ 
    #Please keep this
    list1 = [110,190,440,940,2140,3000,4000,5000]
    ...
    img_save_dir=os.path.join(defaultPath, "saved_ims_v3_June_5_2024","EightPoints_Fit",CIRC_ID)
    ...
    stats_Data_file = os.path.join(os.path.dirname(img_save_dir), "mapping_Data.csv") 
    MP01.export_stats(filename=stats_Data_file,crange=[0,3000])

    for ss,obj in enumerate(MP01_list):
        T1mean=np.mean(obj._map[mask[:,:,ss]])
        T1std=np.std(obj._map[mask[:,:,ss]])


        keys=['CIRC_ID','ID','valueList','shape','finalIndex','T1mean','T1std']
        stats=[obj.CIRC_ID,obj.ID,str(obj.valueList),str(np.shape(obj._data)),obj._finalIndexList[0],T1mean,T1std]
...
if __name__=='__main__':
    ...
    '''
        global list1
        lists=[
            [110,190,440,940,2140,3000,4000,5000],
            [],
            [],
    ...
    with Pool(5) as p:  # Create a pool of 5 processes
        results = p.map(go_fewerPointsFitting,CIRC_ID_List)
